...
**DotNet subcommand with --reference-paths resolves external inheritdoc**: Verifies that
invoking `apimark dotnet` with a repeatable `--reference-paths <path>` flag pointed at an
